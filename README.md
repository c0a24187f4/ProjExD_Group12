...
* 左 CTRL	（通常クリア後のリザルト画面）EXステージへ突入
...
### ボム (BombArea)
* TABキーで使用可能です。
* 使用すると一定時間、自機を中心に円形のバリアが展開され、触れた敵弾を消去します。

### パワーアップ (PowerItem)
* ゲーム中、定期的にパワーアップアイテムが出現します。
* アイテムを取得すると自機がパワーアップし、射撃ダメージが増加します。

### EXステージ
* 通常ステージ（STAGE 1〜3）をクリアした後、リザルト画面でCTRLキーを押すことで突入できる高難易度の隠しステージです。
...
* 通常クリア: STAGE 3をクリアし、リザルト画面でSPACEキーを押します。
* EXクリア: EXステージをクリアし、EXリザルト画面でSPACEキーを押します。
